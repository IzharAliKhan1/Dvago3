...
import { products } from "@/app/users/TopSellingItems/data";
...
export default function TopSellingDetailPage({ params }) {
  const { id } = use(params);
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50 p-4">
        <div className="w-full max-w-md bg-white rounded-3xl shadow-xl p-8 sm:p-10 text-center">
          ...
          <p className="text-gray-600 mb-6 text-sm sm:text-base">The item you are looking for does not exist.</p>
          <Link href="/" className="inline-flex items-center justify-center rounded-full bg-lime-600 px-6 py-3 text-white font-semibold hover:bg-lime-700 text-sm sm:text-base">
            Back to Home
          </Link>
        </div>
      </div>
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-white border-b px-4 sm:px-6 py-2 sm:py-3 text-xs sm:text-sm text-gray-500 overflow-x-auto whitespace-nowrap">
        ...
        <Link href="/" className="hover:text-lime-600">Top Selling Items</Link>
        <span className="mx-1 sm:mx-2">›</span>
      ...
      <div className="max-w-6xl mx-auto px-3 sm:px-4 py-4 sm:py-8">
        ...
        <div className="bg-white rounded-2xl shadow-sm p-4 sm:p-6 grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-10">
          ...
          {/* Image */}
          <div className="relative flex items-center justify-center bg-gray-50 rounded-xl h-56 sm:h-72 md:h-80 border">
          ...
          <div className="space-y-3 sm:space-y-4">
            ...
            <div className="flex items-center flex-wrap gap-2 sm:gap-4">
              ...
              {product.oldPrice && (
                <span className="text-gray-400 line-through text-sm sm:text-lg">{product.oldPrice}</span>
              )}
              {discount && (
        ...
        <div className="mt-6 sm:mt-8">
          <h2 className="text-base sm:text-xl font-bold text-gray-900 border-l-4 border-lime-600 pl-3 mb-3 sm:mb-4">
            Related Products
          </h2>
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3 sm:gap-4">
            {relatedProducts.map((item) => (
              <Link
                key={item.id}
                href={`/topselling/${item.id}`}
                className="bg-white rounded-xl shadow-sm p-2 sm:p-3 hover:shadow-md transition group"
              >
                <div className="relative h-20 sm:h-28 w-full mb-2 sm:mb-3">
                  <Image src={item.image} alt={item.title} fill className="object-contain" />
                </div>
                <p className="text-xs text-gray-700 font-medium line-clamp-2 group-hover:text-lime-600">
                  {item.title}
                </p>
